...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final StitchAppClient client =
                Stitch.initializeDefaultAppClient("cohorto-yzgjh");

        final RemoteMongoClient mongoClient =
                client.getServiceClient(RemoteMongoClient.factory, "mongodb-atlas");

        final RemoteMongoCollection<Document> coll =
                mongoClient.getDatabase("cohorto").getCollection("users");
        ...
        client.getAuth().loginWithCredential(new AnonymousCredential()).continueWithTask(
                new Continuation<StitchUser, Task<RemoteUpdateResult>>() {
                    ...
                    public Task<RemoteUpdateResult> then(@NonNull Task<StitchUser> task) throws Exception {
                        if (!task.isSuccessful()) {
                            Log.e("STITCH", "Login failed!",task.getException().getCause());
            ...
            public void onComplete(@NonNull Task<List<Document>> task) {
                ...
                Log.e("STITCH", "Error: " + task.getException().toString(),task.getException().getCause());
